...
def save_image(image, image_name, prefix="output"):
    ...
    print(f"Da luu anh tai {filename}")

def main(image_path):
    image = cv2.imread(image_path)
    if image is None:
        print(f"Loi khong the doc anh {image_path}")
        return
...
if __name__ == "__main__":
    image_path = './AnhVeTinh_imgs/anh_ve_tinh_1.png'
    image = cv2.imread(image_path)
    if image is None:
        print(f"Loi khong the doc anh tai {image_path}")
    else:
        main(image_path)
